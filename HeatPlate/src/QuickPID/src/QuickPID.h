...
class QuickPID {
  ...
  public:
    ...
    // 预测控制功能 - 高惯性系统优化
    // 启用预测控制功能，设置预测增益系数和接近范围
    void EnablePredictControl(bool enable, float predictGain = 1.0, float approachRange = 5.0);
  ...
  private:
    ...
    // 预测控制相关变量
    bool usePredictControl = false;   // 是否启用预测控制
    float predictGain = 1.0;          // 预测增益系数
    float approachRange = 5.0;        // 接近设定点的范围
